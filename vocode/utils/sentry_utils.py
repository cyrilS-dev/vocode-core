...
from vocode import get_context, serialize_context
...
class SentryConfiguredContextManager:
    ...
    def is_configured(self) -> bool:
        ...
        return client is not None and client.options is not None and "dsn" in client.options
...
def set_tags(span: Span) -> Span:
    serialized_ctx_wrappers = serialize_context()
...
def get_span_by_op(op_value) -> Span:
    transaction: Transaction = get_context("sentry_transaction")
    ...
    elif not hasattr(transaction, '_span_recorder') or transaction._span_recorder is None:
        transaction.init_span_recorder(maxlen=900)
...
def init_span_recorder(self, maxlen: int):
    if not hasattr(self, '_span_recorder') or self._span_recorder is None:
